...
inline void load_strip_design_files()
{
    if(moller_strips_loaded) return;

    std::fstream f1("Moller_GEM_RO_Top_Strip_Coord.txt", std::fstream::in);
    int nb;
    ...
    std::fstream f2("Moller_GEM_RO_Bot_Strip_Coord.txt", std::fstream::in);
...
inline bool has_intersect_top_bot(int i, int j)
{
    if( i<0 || i > 604 || j < 0 || j > 604) {
        std::cout<<"Error: top: "<<i<<" bottom: "<<j<<" not exist."<<endl;
        exit(0);
    }
