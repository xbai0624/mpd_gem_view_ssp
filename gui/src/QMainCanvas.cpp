...
void QMainCanvas::DrawCanvas(const std::string &s, const std::vector<float> &c)
{
    if(c.size() <= 0) 
        return;
    ...
    static TH1I *h = 0;
    if(h == 0){
        h = new TH1I("h", s.c_str(), c.size(), 0, c.size());
    }
    else if((int)c.size() != h->GetNbinsX() ) {
        delete h;
        h = new TH1I("h", s.c_str(), c.size(), 0, c.size());
    }
    h->Reset("M");
        ...
        const std::vector<APVAddress> &addr)
{
    ...
    {
        TH1I * h = new TH1I(Form("h_crate_%d_mpd_%d_adc_ch_%d", addr.crate_id, addr.mpd_id, addr.adc_ch), 
                Form("h_crate_%d_mpd_%d_adc_ch_%d", addr.crate_id, addr.mpd_id, addr.adc_ch), 
                v.size(), 0, v.size());
