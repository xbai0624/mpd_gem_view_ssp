...
do
	...
	"$THIS_DIR/bin/replay" -c 0 -t 0 -z 1 -n -1 --tracking on --pedestal "$THIS_DIR/database/gem_ped_$PEDESTAL.dat" --common_mode "$THIS_DIR/database/CommonModeRange_$PEDESTAL.txt" --gem_map "$THIS_DIR/database/$GEM_MAP" --tracking_config "$THIS_DIR/config/$TRACK_CONFIG" "/mnt/Dual_OS_Drive/Liyanage_MPGD_Files/DAQ_Analysis_Software/data/fermilab_beamtest_$RUN.evio.$splitNum"
	# rename runs
